	...
	$.get('/getFriendRequests', function( data ) {
		...
		if(data.length > 0) {
			for(var i = 0; i < data.length; i++){
				requestsHTML += "<form id='" + data[i].requester + "'>" + data[i].requester + "<button class=\"approve\" id='"  + data[i].requester + "' type=\"button\">Approve</button><button class=\"reject\" type=\"button\">Ignore</button></form>";
			}
